...
module.exports = {
  deleteUser: (res, req, next) => {
    ...
    await User.destroy({ where: { user_id } })
      ...
      .catch(error => {res.status(500).json({message: "Failed to delete User", error})})
  },
  updateUser: (res, req, next) => {
    ...
    await User.update({ email, passwordHash }, { where: { user_id } })
      .then(() => { res.ststus(200).json({ message: "User successfully updated!" }) })
      .catch(error => {res.status(500).json({message: "Failed to update User", error})})
  },
